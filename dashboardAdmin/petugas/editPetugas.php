...
$id_user = mysqli_real_escape_string($connect, $_GET['id'] ?? '');
$res = mysqli_query($connect, "SELECT * FROM users WHERE id_user = '$id_user' AND role = 'petugas'");
$petugas = mysqli_fetch_assoc($res);
...
if(isset($_POST['update'])){
    ...
    $check = mysqli_query($connect, "SELECT * FROM users WHERE username = '$username' AND id_user != '$id_user'");
    if(mysqli_num_rows($check) > 0){
        $error = "Username sudah digunakan!";
    } else {
        $sql = "UPDATE users SET nama = '$nama', username = '$username', no_telp = '$no_telp'";
        if(!empty($password)){
        ...
        if(mysqli_query($connect, $sql)){
            header("Location: petugas.php?msg=edit_success");
            exit;
        } else {
...
<head>
    ...
    <title>Edit Petugas - PopFit Admin</title>
    ...
    <aside class="hidden md:flex flex-col w-64 bg-popfit-dark text-white border-r border-popfit-dark h-full flex-shrink-0">
        ...
        <nav class="flex-1 overflow-y-auto py-4">
            <ul class="space-y-1">
                ...
                <li><a href="petugas.php" class="nav-active flex items-center px-6 py-3 text-gray-200 hover:bg-popfit-light transition-colors border-l-4 border-transparent"><i class="ph ph-user-tie text-xl w-6"></i><span class="ml-3 font-bold">Petugas</span></a></li>
            </ul>
        </nav>
    </aside>
    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden">
        <header class="h-16 bg-popfit-surface border-b border-popfit-border flex items-center justify-between px-6 flex-shrink-0">
            <div class="flex items-center">
                ...
                <h2 class="text-lg font-black text-popfit-dark uppercase tracking-tight">Perbarui Data Petugas</h2>
            </div>
        </header>
                ...
                <form method="POST" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="nama" required value="<?= htmlspecialchars($petugas['nama']) ?>" class="w-full bg-popfit-bg border border-popfit-border rounded-sm px-4 py-3 text-xs font-bold text-popfit-dark focus:border-popfit-dark outline-none transition-all uppercase">
                        </div>
                        <div class="space-y-2">
                            <label class="text-[10px] font-black text-popfit-textMuted uppercase tracking-widest">Username Login</label>
                            <input type="text" name="username" required value="<?= htmlspecialchars($petugas['username']) ?>" class="w-full bg-popfit-bg border border-popfit-border rounded-sm px-4 py-3 text-xs font-bold text-popfit-dark focus:border-popfit-dark outline-none transition-all lowercase">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="no_telp" required value="<?= htmlspecialchars($petugas['no_telp']) ?>" class="w-full bg-popfit-bg border border-popfit-border rounded-sm px-4 py-3 text-xs font-bold text-popfit-dark focus:border-popfit-dark outline-none transition-all">
                        </div>
                    </div>
