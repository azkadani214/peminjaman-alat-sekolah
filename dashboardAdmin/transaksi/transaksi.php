    ...
    <div class="flex-1 flex flex-col h-screen w-full relative">
        <header class="h-16 bg-popfit-surface border-b border-popfit-border flex items-center justify-between px-6 flex-shrink-0">
            <div class="flex items-center flex-1">
                ...
                <div class="relative w-full max-w-xs mr-4 hidden sm:block">
                    <form action="" method="GET" class="relative">
                        <?php if($filterStatus): ?>
                            <input type="hidden" name="status" value="<?= $filterStatus ?>">
                        <?php endif; ?>
                        <i class="ph ph-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                        <input type="text" name="keyword" value="<?= htmlspecialchars($keyword) ?>" placeholder="Cari Siswa/NIS..." class="w-full bg-popfit-bg border border-popfit-border rounded-sm pl-10 pr-4 py-2 text-[11px] font-bold text-popfit-dark focus:border-popfit-dark outline-none transition-all">
                    </form>
                </div>
                <h2 class="text-lg font-black text-popfit-dark uppercase tracking-tight hidden lg:block">Kelola Transaksi</h2>
            </div>
            <div class="flex items-center space-x-2">
                <a href="transaksi.php<?= $keyword ? '?keyword='.$keyword : '' ?>" class="px-3 py-1.5 text-[10px] font-black uppercase <?= !$filterStatus ? 'bg-popfit-dark text-white' : 'bg-white text-popfit-textMuted border border-popfit-border' ?> rounded-sm transition-all">Semua</a>
                <a href="?status=menunggu<?= $keyword ? '&keyword='.$keyword : '' ?>" class="px-3 py-1.5 text-[10px] font-black uppercase <?= $filterStatus=='menunggu' ? 'bg-popfit-dark text-white' : 'bg-white text-popfit-textMuted border border-popfit-border' ?> rounded-sm transition-all">Menunggu</a>
                <a href="?status=dipinjam<?= $keyword ? '&keyword='.$keyword : '' ?>" class="px-3 py-1.5 text-[10px] font-black uppercase <?= $filterStatus=='dipinjam' ? 'bg-popfit-dark text-white' : 'bg-white text-popfit-textMuted border border-popfit-border' ?> rounded-sm transition-all">Aktif</a>
            </div>
        </header>
